...
public class JKSKeyStoreUtil {
	...
	public void importPEM(String pem) throws JKSKeyStoreUtilException, IOException {
		...
		try {
			...
			String fingerprint = getSHA1Fingerprint(cert);
            ...
            while (containsAlias(alias)) {
               ...
               alias += "-dup";
				...
				logger.info("Adding Cert with alias: " + alias + " (Sha1 Fingerprint: " + fingerprint + ")");
...
/*		String alias = "";
   ...
   private String getSHA1Fingerprint(X509Certificate certificate)
   {
      try
      {
         return DigestUtils.shaHex(certificate.getEncoded()).toUpperCase();
      }
...
/*	private String getCertAttribute(X509Certificate certificate, String attributeName) throws NamingException {
	...
	public void importJKSKeyStore(JKSKeyStoreUtil importJKSU) throws KeyStoreException {
		...
		while (aliases.hasMoreElements()) {
			String oldalias = aliases.nextElement();
